    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="/jscss/admin/design/">首页</a><span
                    class="crumb-step">&gt;</span><a class="crumb-name" href="/jscss/admin/design/">寝室管理</a><span
                    class="crumb-step">&gt;</span><span>编辑寝室</span></div>
        </div>
        <div class="result-wrap">
            <div class="result-content">
                <form action="/dormitory_action" method="get" id="myform" name="myform" onsubmit="return write_member()">
                    <table class="insert-tab" width="100%">
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="apartmentId" id="catid" class="required">
                                    ...
                                    %>
                                    <option value="<%=item.getId()%>" <%=dormitory.getApartmentId() == item.getId() ? "selected" : ""%>><%=item.getName()%>
                                    </option>
                                    <%
                                        }
                                    %>
                        ...
                        <tr>
                            <th>寝室名称：</th>
                            <td><input class="common-text" name="name" size="50" value="<%=dormitory.getName()%>"
                                       type="text"></td>
                        </tr>
                        <tr>
                            <th>寝室人员：</th>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input name="id" value="<%=dormitory.getId()%>" type="hidden">
...
<script>
    ...
    let datas = [
        <%
            ...
            for (User item : studentList) {
            	if (dormitory.getMemberUser().contains(item)){
            		continue;
            	}
        %>
    ...
    let datasR = [
        <%
            List<User> memberUser = dormitory.getMemberUser();
            for (User item : memberUser) {
        %>
            {id: "<%=item.getId()%>", name: "<%=item.getName()%>"},
        <%
             }
