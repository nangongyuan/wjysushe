...
<%@ page import="com.wjysushe.bean.Examination" %>
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="index.jsp">首页</a><span
                    class="crumb-step">&gt;</span><span class="crumb-name">查寝管理</span></div>
        </div>
        <div class="search-wrap">
            <div class="search-content">
                <form action="/examination_action" method="get">
            ...
            <form name="myform" id="myform" method="post">
                <div class="result-title">
                    <div class="result-list">
                        <%
                            if (user.getRole()!=3){
                                out.println("<a href=\"/examination_action?action=add\"><i class=\"icon-font\">\uE026</i>新增查寝</a>");
                            }
                        %>
                    </div>
                </div>
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <%
                            List<Examination> examinationList = (List<Examination>) request.getAttribute("list");
                            for (Examination item : examinationList) {
                        %>
                        <tr>
                            <td><%=item.getId()%>
                            </td>
                            ...
                            <td><%=item.getExaminationDate()%>
                            </td>
                            <td><%=item.getPeopleNumber()%>
                            </td>
                            <td><%=item.getRealNumber()%>
                            </td>
                            <td><%=item.getStudentNames()%>
                            </td>
                            <%--<td>--%>
                                <%--<a class="link-update" href="/examination_action?action=edit&id=<%=item.getId()%>">修改</a>--%>
                                <%--<a class="link-del" href="/examination_action?action=delete&id=<%=item.getId()%>"--%>
