layui.define(['jquery', 'laytpl', 'layer'], function (e) {
    ...
    job.prototype.pullData = function () {
        ...
        (!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax({
            ...
            url: _config.url,
            data: {term: _config.filter},
            dataType: "json",
            beforeSend: function () {
              _elem.attr('data-patent-first','')
              _elem.attr('data-patent-second','')
            ...
            success: function (resp) {
              // return resp.code!=0 ? layer.msg(resp.msg) : _config.data[_self.index] = resp.data, _self.renderData(_config.data[_self.index])
                return 0 != eval('resp.' + _config.response.code) ? layer.msg('请求失败') : _config.data[_self.index] = eval('resp.' + _config.response.data), _self.renderData(_config.data[_self.index])
            },
            error: function () {
                hint.error("请求失败")
            },
    ...
    job.prototype.events = function () {
        ...
        _elem.on('focus', function () {
            ...
            clearTimeout(_config._ajax);
          _config._ajax = setTimeout(function () {
        ...
        $(document).on('click', function (e) {
            ...
            if (_e !== undefined) {
                ...
                var temp_arr = Object.values(_config.data[_self.index][_e.index()])
              _elem.attr('data-patent-first',temp_arr[0])
              if (temp_arr.length>1){
                _elem.attr('data-patent-second',temp_arr[1])
              }
            }else if (_elem.attr('fillin')=='false' && _elem.attr('data-patent-first')==''){
              _elem.val('');
            }
